...
addonObject = addon('script.module.promisescrapers')
...
def autoTraktSubscription(tvshowtitle, year, imdb, tvdb):
    from promisescrapers.modules import libtools
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.thepromise.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.thepromise.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.thepromise.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.thepromise.artwork)')
...
# def metaFile():
    # if condVisibility('System.HasAddon(script.thepromise.metadata)'):
        # return os.path.join(xbmcaddon.Addon('script.thepromise.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
