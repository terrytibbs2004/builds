...
DEBUGPREFIX = '[ ThePromise {0} | {1} | {2} | DEBUG ]'.format(version, kodi_version, sys_platform)
INFOPREFIX = '[ ThePromise | INFO ]'
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('ThePromise Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
...
def upload_log():
    ...
    if not data:
        ...
        UserAgent = 'ThePromise %s' % version
        try:
