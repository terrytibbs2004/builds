...
DEBUGPREFIX = '[ homelander {0} | {1} | {2} | DEBUG ]'.format(version, kodi_version, sys_platform)
INFOPREFIX = '[ homelander | INFO ]'
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('homelander Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
...
def upload_log():
    ...
    if not data:
        ...
        UserAgent = 'homelander %s' % version
        try:
