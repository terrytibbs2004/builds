...
addonObject = addon('script.module.homelanderscrapers')
...
def autoTraktSubscription(tvshowtitle, year, imdb, tvdb):
    from homelanderscrapers.modules import libtools
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.homelander.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.homelander.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.homelander.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.homelander.artwork)')
...
# def metaFile():
    # if condVisibility('System.HasAddon(script.homelander.metadata)'):
        # return os.path.join(xbmcaddon.Addon('script.homelander.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
