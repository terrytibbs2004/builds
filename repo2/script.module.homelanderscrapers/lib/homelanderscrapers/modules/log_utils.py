...
DEBUGPREFIX = '[ homelanderScrapers {0} | DEBUG ]'.format(version)
INFOPREFIX = '[ homelanderScrapers | INFO ]'
LOGPATH = control.transPath('special://logpath/')
log_file = os.path.join(LOGPATH, 'homelander.log')
debug_enabled = control.addon('plugin.video.homelander').getSetting('addon.debug')
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('homelanderScrapers Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
