...
DEBUGPREFIX = '[ nightwingscrapers {0} | DEBUG ]'.format(version)
INFOPREFIX = '[ nightwingscrapers | INFO ]'
LOGPATH = control.transPath('special://logpath/')
log_file = os.path.join(LOGPATH, 'theoath.log')
debug_enabled = control.addon('plugin.video.nightwing').getSetting('addon.debug')
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('nightwingscrapers Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
