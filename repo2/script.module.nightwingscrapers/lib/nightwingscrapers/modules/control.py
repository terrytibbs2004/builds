...
addonObject = addon('script.module.nightwingscrapers')
...
def autoTraktSubscription(tvshowtitle, year, imdb, tvdb):
    from nightwingscrapers.modules import libtools
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.Nightwing.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.Nightwing.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.Nightwing.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.Nightwing.artwork)')
...
# def metaFile():
    # if condVisibility('System.HasAddon(script.Nightwing.metadata)'):
        # return os.path.join(xbmcaddon.Addon('script.Nightwing.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
