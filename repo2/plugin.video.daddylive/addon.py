...
def Main_Menu():
    menu=[
        ['LIVE TV','live_tv'],
        ['LIVE SPORTS -  Greece Time → GMT+2','sched'],
    ...
    for m in menu:
        ...
        li.setArt({'icon':addon.getAddonInfo("path") + '/resources/media/icon.png'})
        li.setArt({'fanart':addon.getAddonInfo("path") + '/resources/media/fanart.jpg'})
...
def getCategTrans():
    resp=requests.get('https://daddylive.eu').text
    blocks=resp.split('role=\"alert\"')
    categs=[]
    for b in blocks:
...
def getTransData(categ):
    ...
    for b in blocks:#transmisje dla danej kategorii
        if 'noopener' in b and '<h4>' in b and categ+'</span></h4>' in b:
            ar_tr=[]
            items=b.split('</p>')#<br />
            for i in items:
                ii=i.split('<br />')
...
def TransList(categ):
    ...
    for t in trns:
        ...
        if len(t[1])==1:
            url_stream = build_url({'mode':'play','url':t[1][0]})
...
def channels():
    url='https://daddylive.eu/24-hours-channels.php'
...
def PlayStream(link):
    ...
    url_2=re.compile('iframe src="(.*)" width').findall(resp)[0]
    #print(url_2)
    ...
    resp=requests.post(url_2, headers=hea).text
    stream=re.compile('source:\'(.*)\'').findall(resp)[-1]
    stream_url=stream
    hdr='Referer='+quote(str(url_2))+'&User-Agent='+quote('Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:98.0) Gecko/20100101 Firefox/98.0')
