...
from jetextractors import extractors, extractor
...
SEARCH_INCLUDE = ["Full Match TV"]

addon_fanart = xbmcaddon.Addon().getAddonInfo('fanart')
addon_icon = xbmcaddon.Addon().getAddonInfo('icon')

class FullSearch(Plugin):
    name = "fullsearch"
    priority = 100
    ...
    def routes(self, plugin):
        ...
        def search(query):
            ...
            for game in games:
                ...
                #}
                jen_data = {
                    "title": "[COLORdodgerblue][COLORwhite][B] %s [/B][COLORred] %s | %s[/COLOR]" % (game.title, game.extractor, format_time(game.starttime)),
                    "thumbnail": game.icon,
                    "fanart": game.icon,
                    "summary": game.title,
                    "sportjetextractors": [link.to_dict() for link in game.links],
            ...
            jen_list = sorted(jen_list, key=lambda x: x["time"], reverse=True)
