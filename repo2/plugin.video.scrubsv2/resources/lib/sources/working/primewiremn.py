...
class source:
    ...
    def tvshow(self, imdb, tvdb, tvshowtitle, localtvshowtitle, aliases, year):
        try:
            ...
            if tvshowtitle == 'House':
                aliases.append({'country': 'US', 'title': 'House M.D.'})
    ...
    def sources(self, url, hostDict):
        try:
            ...
            self.cookie = client.request(self.base_link, output='cookie', timeout='6')
            html = client.request(search_url, cookie=self.cookie, timeout='6')
            ...
            if 'tvshowtitle' in data:
                ...
                r = client.request(url, cookie=self.cookie, timeout='6')
    ...
    def resolve(self, url):
        if any(x in url for x in self.domains):
            try:
                r = client.request(url, timeout='6')
                try:
                    v = re.findall(r'document.write\(Base64.decode\("(.+?)"\)', r)[0]
                    b64 = base64.b64decode(v)
                    b64 = ensure_text(b64, errors='ignore')
