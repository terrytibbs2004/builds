...
class source:
    def __init__(self):
        self.results = []
        self.domains = ['yify.plus', 'yifytv.top']
        self.base_link = 'https://yify.plus'
    ...
    def sources(self, url, hostDict):
        try:
            ...
            for data_type, data_post, data_nume in results:
                ...
                log_utils.log('Scraper yifyplus sources i: ' + repr(i))
                p = client.parseDOM(i.replace('\\', ''), 'iframe', ret='src')
                for link in p:
