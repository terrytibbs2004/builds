...
def downloader():
    ...
    if status == 'downloading':
        item = control.item('[COLOR lime]Stop Downloads[/COLOR]', iconImage=thumb, thumbnailImage=thumb)
        item.addContextMenuItems([], replaceItems=True)
        item.setProperty('fanart_image', fanart)
    ...
    if status == 'downloading':
        item = control.item('[COLOR lime]Download Status[/COLOR]', iconImage=thumb, thumbnailImage=thumb)
        item.addContextMenuItems([], replaceItems=True)
        item.setProperty('Fanart_Image', fanart)
...
class MyDownloads(pyxbmct.AddonDialogWindow):
    ...
    def set_info_controls(self):
        self.placeControl(pyxbmct.Label('[B][COLOR lime]'+control.window.getProperty(property + '.name')+'[/COLOR][/B]', alignment=pyxbmct.ALIGN_CENTER), 0, 1, 1, 1)
