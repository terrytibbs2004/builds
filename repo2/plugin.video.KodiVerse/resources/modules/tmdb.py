       ...
       #except:
           ...
           for data in result_html:
             ...
             if (new_name+id) not in new_name_array:
              ...
              if  Addon.getSetting("disapear")=='true' and color=='lime' and mode!=7:
                a=1
              else:
                color='lime'
...
def get_movies(url,local=False,reco=0,global_s=False,return_results=False):
        ...
        for items in all_years:
            ...
            if 0:
               if  name not in all_n:
                ...
                aa=addDir3('[COLOR lime]'+items+'[/COLOR]',url,14,'https://i.imgur.com/vWRnI5B.png','https://i.imgur.com/t0PWTDF.jpg',items,collect_all=True)
                all_d.append(aa)
            else:
                aa=addDir3('[COLOR lime]'+items+'[/COLOR]',url,14,'https://i.imgur.com/vWRnI5B.png','https://i.imgur.com/t0PWTDF.jpg',items,collect_all=True)
                all_d.append(aa)
   ...
   if   max_page==-1 and not return_results:
        if not global_s:
            xbmcgui.Dialog().ok(Addon.getAddonInfo('name'),'[COLOR lime][I]%s[/I][/COLOR]'%Addon.getLocalizedString(32183))
   ...
   for data in html['seasons']:
     ...
     try:
        if 'air_date' in data:
               ...
               datea='[COLOR lime]'+str(time.strptime(data['air_date'], '%Y-%m-%d'))+'[/COLOR]\n'
               ...
               if a>b:
                 ...
                 color='lime'
        datea='[COLOR lime]'+txt_1+time.strftime( "%d-%m-%Y",a) + '[/COLOR]\n'
     except Exception as e:
...
def get_episode(name,url,iconimage,fanart,description,data,original_title,id,season,tvdb_id,show_original_year,heb_name):
   ...
   for data in html['episodes']:
     ...
     try:
        ...
        if 'air_date' in data:
               ...
               if a>b:
                 ...
                 color2='lime'
        datea='[COLOR lime]'+Addon.getLocalizedString(32187)+time.strftime( "%d-%m-%Y",a) + '[/COLOR]\n'
     except:
             try:
                datea=data['air_date']
             except:
     ...
     xxx=xxx+1
     if  Addon.getSetting("disapear")=='true' and color=='lime':
        a=1
     else:
