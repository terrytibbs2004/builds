...
DEBUGPREFIX = '[ PromiseScrapers {0} | DEBUG ]'.format(version)
INFOPREFIX = '[ PromiseScrapers | INFO ]'
LOGPATH = control.transPath('special://logpath/')
log_file = os.path.join(LOGPATH, 'thepromise.log')
debug_enabled = control.addon('plugin.video.thepromise').getSetting('addon.debug')
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('PromiseScrapers Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
