...
addonObject = addon('script.module.ninescrapers')
...
def autoTraktSubscription(tvshowtitle, year, imdb, tvdb):
    from ninescrapers.modules import libtools
...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.nine.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.nine.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.nine.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.nine.artwork)')
...
# def metaFile():
    # if condVisibility('System.HasAddon(script.nine.metadata)'):
        # return os.path.join(xbmcaddon.Addon('script.nine.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
