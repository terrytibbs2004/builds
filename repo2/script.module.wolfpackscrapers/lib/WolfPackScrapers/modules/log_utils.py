...
DEBUGPREFIX = '[ Addon Template DEBUG ]'
LOGPATH = control.transPath('special://logpath/')
log_file = os.path.join(LOGPATH, 'pressplay.log')
debug_enabled = control.addon('plugin.video.wolfpack').getSetting('addon.debug')
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('Addon Template Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
