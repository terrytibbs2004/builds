...
from urllib.parse import urlparse, urljoin, quote, urlencode, quote_plus, unquote_plus
...
INCLUDETHECREW  = ADDON.getSetting('includethecrew')
INCLUDEHOMELANDER  = ADDON.getSetting('includehomelander')
INCLUDEMORIA  = ADDON.getSetting('includemoria')
...
def checkBuild(name, ret, burl=None):
    ...
    bf = textCache(burl).decode("utf-8")
...
def checkTheme(name, theme, ret):
    ...
    link = openURL(themeurl).decode('utf-8').replace('\n','').replace('\r','').replace('\t','')
...
def checkWizard(ret):
    ...
    link = openURL(WIZARDFILE).decode('utf-8').replace('\n','').replace('\r','').replace('\t','')
...
def checkInfo(url):
    ...
    link = openURL(url).decode('utf-8').replace('\n','').replace('\r','').replace('\t','')
...
def buildCount(ver=None, url=None):
    ...
    total = 0; count15 = 0; count16 = 0; count17 = 0; count18 = 0; count19 = 0; count20 = 0; hidden = 0; adultcount = 0
    if len(match) > 0:
        for name, kodi, adult in match:
            ...
            kodi = int(float(kodi))
            total += 1
            if kodi == 20: count20 += 1
            elif kodi == 19: count19 += 1
            elif kodi == 18: count18 += 1
            elif kodi == 17: count17 += 1
            elif kodi == 16: count16 += 1
            elif kodi <= 15: count15 += 1
    return total, count15, count16, count17, count18, count19, count20, adultcount, hidden
...
def themeCount(name, count=True):
    ...
    link = openURL(themefile).decode('utf-8').replace('\n','').replace('\r','').replace('\t','')
...
def textCache(url):
    ...
    if CACHETEXT.lower() == 'yes':
        spliturl = url.split('/')
        if not os.path.exists(TEXTCACHE): os.makedirs(TEXTCACHE)
        file = legalFilename(os.path.join(TEXTCACHE, spliturl[-2]+'.txt'))
        ...
        if not os.path.exists(file):
            ...
            content = base64.b64encode(textfile)
            f.write(content)
            f.close()
...
def getCacheSize():
    ...
    dbfiles   = [
        ...
        (os.path.join(ADDOND, 'plugin.video.homelander', 'cache.db')),
        (os.path.join(ADDOND, 'plugin.video.homelander', 'cache.meta.5.db')),
        (os.path.join(ADDOND, 'plugin.video.homelander', 'cache.providers.13.db')),
        (os.path.join(ADDOND, 'plugin.video.moria', 'cache.db')),
        (os.path.join(ADDOND, 'plugin.video.moria', 'cache.meta.5.db')),
        (os.path.join(ADDOND, 'plugin.video.moria', 'cache.providers.13.db')),
    ...
    if INCLUDEVIDEO == 'true':
        ...
        if INCLUDEALL == 'true': files = dbfiles
            ...
            if INCLUDETHECREW == 'true': 
                files.append(os.path.join(ADDOND, 'plugin.video.thecrew', 'cache.db'))
                files.append(os.path.join(ADDOND, 'plugin.video.thecrew', 'meta.5.db'))
                files.append(os.path.join(ADDOND, 'plugin.video.thecrew', 'providers.13.db'))
            if INCLUDEHOMELANDER == 'true': 
                files.append(os.path.join(ADDOND, 'plugin.video.homelander', 'cache.db'))
                files.append(os.path.join(ADDOND, 'plugin.video.homelander', 'meta.5.db'))
                files.append(os.path.join(ADDOND, 'plugin.video.homelander', 'providers.13.db'))
            if INCLUDEMORIA == 'true': 
                files.append(os.path.join(ADDOND, 'plugin.video.moria', 'cache.db'))
                files.append(os.path.join(ADDOND, 'plugin.video.moria', 'meta.5.db'))
                files.append(os.path.join(ADDOND, 'plugin.video.moria', 'providers.13.db'))
        if len(files) > 0:
            for item in files:
                if not os.path.exists(item): continue
                totalsize += os.path.getsize(item)
        else: log("Clear Cache: Clear Video Cache Not Enabled", LOGNOTICE)
    return totalsize
...
def removeFolder(path):
    log("Deleting Folder: %s" % path, LOGNOTICE)
    try: shutil.rmtree(path,ignore_errors=True, onerror=None)
    except: return False

def removeFile(path):
    log("Deleting File: %s" % path, LOGNOTICE)
...
def emptyfolder(folder):
    ...
    for root, dirs, files in os.walk(folder, topdown=True):
        ...
        if file_count == 0:
            ...
            total += 1
            log("Empty Folder: %s" % root, LOGNOTICE)
    return total

def log(msg, level=xbmc.LOGDEBUG):
    if not os.path.exists(ADDONDATA): os.makedirs(ADDONDATA)
    if not os.path.exists(WIZLOG): f = xbmcvfs.File(WIZLOG, 'w'); f.close()
    if WIZDEBUGGING == 'false': return False
    if DEBUGLEVEL == '0': return False
    if DEBUGLEVEL == '1' and not level in [LOGNOTICE, xbmc.LOGERROR, xbmc.LOGSEVERE, xbmc.LOGFATAL]: return False
    if DEBUGLEVEL == '2': level = LOGNOTICE
    try:
    ...
    if ENABLEWIZLOG == 'true':
        ...
        with xbmcvfs.File(WIZLOG, 'w') as f:
...
def checkLog():
    ...
    if CLEANWIZLOGBY == '0':
        ...
        x    = 0
        f      = open(WIZLOG, 'rb'); a = f.read().decode('utf-8', errors="ignore"); f.close(); lines = a.split('\n')
        for line in lines:
        ...
        newfile = lines[x:]
        writing = '\n'.join(newfile)
        f = xbmcvfs.File(WIZLOG, 'w'); f.write(writing); f.close()
...
def toggleDependency(name, DP=None):
    ...
    if os.path.exists(dep):
        source = xbmcvfs.File(dep); link=source.read(); source.close()
        match  = parseDOM(link, 'import', ret='addon')
        for depends in match:
            if not 'xbmc.python' in depends:
                dependspath=os.path.join(ADDONS, depends)
                if not DP == None:
                    DP.update("","Checking Dependency [COLOR yellow]%s[/COLOR] for [COLOR yellow]%s[/COLOR][CR]" % (depends, name))
...
def toggleAdult():
    ...
    link = openURL('http://noobsandnerds.com/TI/AddonPortal/adult.php').decode('utf-8').replace('\n','').replace('\r','').replace('\t','')
    ...
    for item in list:
        ...
        if os.path.exists(fold):
            ...
            log("[Toggle Adult] %s %s" % (goto, item), LOGNOTICE)
    if len(found) > 0:
...
def createTemp(plugin):
    ...
    f      = xbmcvfs.File(temp); r = f.read(); f.close()
    plugdir = os.path.join(ADDONS, plugin)
    if not os.path.exists(plugdir): os.makedirs(plugdir)
    a = xbmcvfs.File(os.path.join(plugdir, 'addon.xml'), 'w')
...
def toggleAddon(id, value, over=None):
    ...
    if os.path.exists(addonxml):
        ...
        try:
            if len(tid) > 0:
                addonid = tid[0]
            if len(tservice) > 0:
                log("We got a live one, stopping script: %s" % tservice[0], xbmc.LOGDEBUG)
    ...
    if 'error' in response and over == None:
        ...
        DIALOG.ok(ADDONTITLE, "[COLOR %s]Error %s [COLOR %s]%s[/COLOR][CR]Check to make sure the addon list is upto date and try again.[/COLOR]" % (COLOR2, v, COLOR1 , id))
        forceUpdate()
...
def createQR():
    ...
    if not working == True:
        if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]It seems the your enter isnt working, Would you like to create it anyways?[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, working), yeslabel="[B][COLOR red]Yes Create[/COLOR][/B]", nolabel="[B][COLOR springgreen]No Cancel[/COLOR][/B]"):
            return
    ...
    DIALOG.ok(ADDONTITLE, "[COLOR %s]The QRCode image has been created and is located in the addondata directory:[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, image.replace(HOME, '')))
...
def cleanupBackup():
    ...
    if selected == -1:
        ...
        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to clean up all items in your 'My_Builds' folder?[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, MYBUILDS), yeslabel="[B][COLOR springgreen]Clean Up[/COLOR][/B]", nolabel="[B][COLOR red]No Cancel[/COLOR][/B]"):
            clearedfiles, clearedfolders = cleanHouse(transPath(MYBUILDS))
        ...
        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to remove [COLOR %s]%s[/COLOR] from 'My_Builds' folder?[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, list[selected], COLOR1, path), yeslabel="[B][COLOR springgreen]Clean Up[/COLOR][/B]", nolabel="[B][COLOR red]No Cancel[/COLOR][/B]"):
            if os.path.isfile(path):
                ...
                try:
                    ...
                    log("Error removing %s" % path, LOGNOTICE)
...
def splitNotify(notify):
    link = openURL(notify).decode('utf-8').replace('\r','').replace('\t','').replace('\n', '[CR]')
...
def force_check_updates(auto=False, over=True, sec=20):
    if not over:
        LogNotify(ADDONTITLE,
                           '[COLOR {0}]Force Checking for Updates[/COLOR]'.format(COLOR2))

    dbfile = latestDB('Addons')
    dbfile = os.path.join(DATABASE, dbfile)
    textdb = database.connect(dbfile)
    textexe = textdb.cursor()
    textexe.execute("UPDATE repo SET version = ?, checksum = ?, lastcheck = ?", ('', '', '',))
    textdb.commit()

    xbmc.executebuiltin('UpdateAddonRepos')

    with busy_dialog():
        installed_repos = textexe.execute('SELECT addonID FROM repo')

        start_time = time.time()
        checked_time = 0
        for repo in installed_repos.fetchall():
            repo = repo[0]
            log('Force checking {0}...'.format(repo), level=xbmc.LOGDEBUG)
            while checked_time < start_time:
                if time.time() >= start_time + sec:
                    log('{0} timed out during repo force check.'.format(repo), level=xbmc.LOGDEBUG)
                    break
                
                lastcheck = textexe.execute('SELECT lastcheck FROM repo WHERE addonID = ?', (repo,))
                ...
                xbmc.sleep(1000)
            checked_time = 0
            log('{0} successfully force checked.'.format(repo), level=xbmc.LOGDEBUG)
        if not over:
            LogNotify('[COLOR {0}]{1}[/COLOR]'.format(COLOR1, ADDONTITLE),
                               "[COLOR {0}]All repos force checked for updates.[/COLOR]".format(COLOR2))
    textexe.close()
...
def convertSpecial(url, over=False):
    ...
    DP.create(ADDONTITLE, "[COLOR %s]Changing Physical Paths To Special[CR][CR]Please Wait[/COLOR]" % COLOR2)
    for root, dirs, files in os.walk(url):
        for file in files:
            start += 1
            perc = int(percentage(start, total))
            if file.endswith(".xml") or file.endswith(".hash") or file.endswith(".properties"):
                DP.update(perc, "[COLOR %s]Scanning: [COLOR %s]%s[/COLOR][CR][COLOR %s]%s[/COLOR][CR]Please Wait[/COLOR]" % (COLOR2, COLOR1, root.replace(HOME, ''), COLOR1, file))
                try:
                    with xbmcvfs.File(os.path.join(root, file), "r") as a:
                        a = a.read()
                    encodedpath  = quote(HOME)
                    encodedpath2  = quote(HOME).replace('%3A','%3a').replace('%5C','%5c')
                    encodedpath3  = quote(HOME).replace('%3A',':').replace('%5C','\\\\')
                    b = a.replace(HOME, 'special://home/').replace(encodedpath, 'special://home/').replace(encodedpath2, 'special://home/').replace(encodedpath3, 'special://home/')
                    with xbmcvfs.File(os.path.join(root, file), "w") as f:
                        f.write(str(b))
                    if DP.iscanceled():
                        DP.close()
                    ...
                    LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "Problem converting paths in %s" % os.path.join(root, file))
                    continue
    DP.close()
    log("[Convert Paths to Special] Complete", LOGNOTICE)
...
def clearCrash():
    ...
    for file in glob.glob(os.path.join(LOG, '*crashlog*.*')):
        files.append(file)
    if len(files) > 0:
        if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to delete the Crash logs?[CR][COLOR %s]%s[/COLOR] Files Found[/COLOR]' % (COLOR2, COLOR1, len(files)), yeslabel="[B][COLOR springgreen]Remove Logs[/COLOR][/B]", nolabel="[B][COLOR red]Keep Logs[/COLOR][/B]"):
            for f in files:
                xbmcvfs.delete(f)
...
def hidePassword():
    if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to [COLOR %s]hide[/COLOR] all passwords when typing in the add-on settings menus?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR springgreen]Hide Passwords[/COLOR][/B]", nolabel="[B][COLOR red]No Cancel[/COLOR][/B]"):
        count = 0
        for folder in glob.glob(os.path.join(ADDONS, '*/')):
            sett = os.path.join(folder, 'resources', 'settings.xml')
            if os.path.exists(sett):
                f = xbmcvfs.File(sett).read()
                    ...
                    if 'pass' in line:
                        if not 'option="hidden"' in line:
                            try:
                                ...
                                pass
                f2 = xbmcvfs.File(sett, 'w'); f2.write(f); f2.close()
        LogNotify("[COLOR %s]Hide Passwords[/COLOR]" % COLOR1, "[COLOR %s]%s items changed[/COLOR]" % (COLOR2, count))
        log("[Hide Passwords] %s items changed" % count, LOGNOTICE)
    else: log("[Hide Passwords] Cancelled", LOGNOTICE)
...
def unhidePassword():
    if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to [COLOR %s]unhide[/COLOR] all passwords when typing in the add-on settings menus?[/COLOR]" % (COLOR2, COLOR1), yeslabel="[B][COLOR springgreen]Unhide Passwords[/COLOR][/B]", nolabel="[B][COLOR red]No Cancel[/COLOR][/B]"):
        ...
        log("[Unhide Passwords] %s items changed" % count, LOGNOTICE)
    else: log("[Unhide Passwords] Cancelled", LOGNOTICE)
...
def wizardUpdate(startup=None):
    if workingURL(WIZARDFILE):
        try:
            ...
            return
        if ver > VERSION:
            yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]There is a new version of the [COLOR %s]%s[/COLOR]![CR]Would you like to download [COLOR %s]v%s[/COLOR]?[/COLOR]' % (COLOR2, COLOR1, ADDONTITLE, COLOR1, ver), nolabel='[B][COLOR red]Remind Me Later[/COLOR][/B]', yeslabel="[B][COLOR springgreen]Update Wizard[/COLOR][/B]")
            if yes:
                ...
                log("[Auto Update Wizard] Wizard updated to v%s" % ver, LOGNOTICE)
                #reloadProfile()
                if startup:
                    ebi('RunScript(%s/startup.py)' % PLUGIN)
                return
            else: log("[Auto Update Wizard] Install New Wizard Ignored: %s" % ver, LOGNOTICE)
        else:
            if not startup: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]No New Version of Wizard[/COLOR]" % COLOR2)
            log("[Auto Update Wizard] No New Version v%s" % ver, LOGNOTICE)
    else: log("[Auto Update Wizard] Url for wizard file not valid: %s" % WIZARDFILE, LOGNOTICE)
...
def convertText():
    ...
    if not BUILDFILE == 'http://':
        ...
        writing = '';x = 0
        a = openURL(BUILDFILE).decode('utf-8').replace('\n','').replace('\r','').replace('\t','')
        DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]Builds.txt[/COLOR][CR][CR]Please Wait' % (COLOR2, COLOR1))
        ...
        if len(match2) == 0:
            for name, version, url, gui, kodi, theme, icon, fanart in match:
                x += 1
                DP.update(int(percentage(x, len(match2))), '[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]Builds.txt[/COLOR][CR][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, COLOR1, name))
                ...
                if not theme == 'http://':
                    ...
                    themewrite = ''; x2 = 0
                    a = openURL(theme).decode('utf-8').replace('\n','').replace('\r','').replace('\t','')
                    DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]%s_theme.txt[/COLOR][CR][CR]Please Wait' % (COLOR2, COLOR1, name))
                    match3 = re.compile('name="(.+?)".+?rl="(.+?)".+?con="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(a)
                    for name, url, icon, fanart, description in match3:
                        x2 += 1
                        DP.update(int(percentage(x2, len(match2))), "[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]%s_theme.txt[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, name, COLOR1, name))
                    ...
                    f = xbmcvfs.File(filename2, 'w'); f.write(themewrite); f.close()
        else:
            for name, version, url, gui, kodi, theme, icon, fanart, preview, adult, description in match2:
                x += 1
                DP.update(int(percentage(x, len(match2))), "[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]Builds.txt[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR1, name))
                if not writing == '': writing += '\n'
                writing += 'name="%s"\n' % name
                writing += 'version="%s"\n' % version
                writing += 'url="%s"\n' % url
                ...
                if not theme == 'http://':
                    ...
                    for name, url, icon, fanart, description in match3:
                        ...
                        themewrite += 'description="%s"\n' % description
                    f = xbmcvfs.File(filename2, 'w'); f.write(themewrite); f.close()
        f = xbmcvfs.File(filename, 'w'); f.write(writing); f.close()
    if not APKFILE == 'http://':
        filename = os.path.join(TEXTFILES, 'apks.txt')
        writing = ''; x = 0
        a = openURL(APKFILE).decode('utf-8').replace('\n','').replace('\r','').replace('\t','')
        DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]Apks.txt[/COLOR][CR][CR]Please Wait' % (COLOR2, COLOR1))
        ...
        if len(match2) == 0:
            for name, url, icon, fanart in match:
                x += 1
                DP.update(int(percentage(x, len(match))), "[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]Apks.txt[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, COLOR1, name))
                if not writing == '': writing += '\n'
                writing += 'name="%s"\n' % name
                writing += 'section="no"'
                writing += 'url="%s"\n' % url
            ...
            for name, url, icon, fanart, adult, description in match2:
                x += 1
                DP.update(int(percentage(x, len(match2))), "[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]Apks.txt[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, COLOR1, name))
                if not writing == '': writing += '\n'
                writing += 'name="%s"\n' % name
                writing += 'section="no"'
                writing += 'url="%s"\n' % url
    ...
    if not YOUTUBEFILE == 'http://':
        ...
        writing = ''; x = 0
        a = openURL(YOUTUBEFILE).decode('utf-8').replace('\n','').replace('\r','').replace('\t','')
        DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]YouTube.txt[/COLOR][CR][CR]Please Wait' % (COLOR2, COLOR1))
        match = re.compile('name="(.+?)".+?rl="(.+?)".+?con="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(a)
        for name, url, icon, fanart, description in match:
            x += 1
            DP.update(int(percentage(x, len(match))), "[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]YouTube.txt[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1 ,COLOR1, name))
            if not writing == '': writing += '\n'
            writing += 'name="%s"\n' % name
            writing += 'section="no"'
            writing += 'url="%s"\n' % url
    ...
    if not ADVANCEDFILE == 'http://':
        ...
        writing = ''; x = 0
        a = openURL(ADVANCEDFILE).decode('utf-8').replace('\n','').replace('\r','').replace('\t','')
        DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]AdvancedSettings.txt[/COLOR][CR][CR]Please Wait' % (COLOR2, COLOR1))
        match = re.compile('name="(.+?)".+?rl="(.+?)".+?con="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(a)
        for name, url, icon, fanart, description in match:
            x += 1
            DP.update(int(percentage(x, len(match))), "[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]AdvancedSettings.txt[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, COLOR1, name))
            if not writing == '': writing += '\n'
            writing += 'name="%s"\n' % name
            writing += 'section="no"'
            writing += 'url="%s"\n' % url
...
def asciiCheck(use=None, over=False):
    ...
    afiles       = xbmcvfs.File(files_found, 'w')
    afails       = xbmcvfs.File(files_fails, 'w')
    f1           = 0; f2           = 0
    items        = fileCount(source)
    msg          = ''
    prog         = []
    log("Source file: (%s)" % str(source), LOGNOTICE)
    ...
    for base, dirs, files in os.walk(source):
        ...
        for file in files:
            ...
            DP.update(prog2,"[COLOR %s]Checking for non ASCII files[CR][COLOR %s]%s[/COLOR][CR]Please Wait[/COLOR]" % (COLOR2, COLOR1, file))
            try:
                file.encode('ascii')
    ...
    total = int(f1) + int(f2)
    if total > 0:
        if os.path.exists(files_found): afiles = xbmcvfs.File(files_found); msg = afiles.read(); afiles.close()
        if os.path.exists(files_fails): afails = xbmcvfs.File(files_fails); msg2 = afails.read(); afails.close()
        if yes:
            if use:
...
def defaultSkin():
    log("[Default Skin Check]", LOGNOTICE)
    ...
    log("Reading gui file: %s" % gui, LOGNOTICE)
    guif = open(gui, 'r+', encoding='utf-8', errors='ignore')
    msg = guif.read().replace('\n','').replace('\r','').replace('\t','').replace('    ',''); guif.close()
    log("Opening gui settings", LOGNOTICE)
    match = re.compile('<lookandfeel>.+?<ski.+?>(.+?)</skin>.+?</lookandfeel>').findall(msg)
    log("Matches: %s" % str(match), LOGNOTICE)
    if len(match) > 0:
        skinid = match[0]
        addonxml = os.path.join(ADDONS, match[0], 'addon.xml')
        if os.path.exists(addonxml):
            addf = open(addonxml, 'r+', encoding='utf-8', errors='ignore')
        ...
        log("[Default Skin Check] Skin name: %s" % skinname, LOGNOTICE)
        log("[Default Skin Check] Skin id: %s" % skinid, LOGNOTICE)
        setS('defaultskin', skinid)
        setS('defaultskinname', skinname)
        setS('defaultskinignore', 'false')
    if os.path.exists(tempgui):
        log("Deleting Temp Gui File.", LOGNOTICE)
        xbmcvfs.delete(tempgui)
    log("[Default Skin Check] End", LOGNOTICE)
    ...
    if do == 'save':
        for item in scan:
            ...
            if not 'error' in response:
                ...
                log("%s saved to %s" % (item, match[0]), LOGNOTICE)
    else:
        for item in scan:
            ...
            response = xbmc.executeJSONRPC(query)
            log("%s restored to %s" % (item, value), LOGNOTICE)
...
def backUpOptions(type, name=""):
    exclude_dirs  = [ADDON_ID, 'cache', 'system', 'Thumbnails', 'peripheral_data', 'temp', 'My_Builds', 'keymaps', 'cdm', '__pycache__']
    exclude_files = ['Textures13.db', '.DS_Store', 'advancedsettings.xml', 'Thumbs.db', '.gitignore']
    ## TODO: fix these
    bad_files     = [
                    (os.path.join(ADDOND, 'plugin.video.thecrew', 'cache.db')),
    ...
    backup   = transPath(BACKUPLOCATION)
    mybuilds = transPath(MYBUILDS)
    try:
        if not os.path.exists(backup): xbmcvfs.mkdirs(backup)
        if not os.path.exists(mybuilds): xbmcvfs.mkdirs(mybuilds)
    except Exception as e:
        DIALOG.ok(ADDONTITLE, "[COLOR %s]Error making Back Up directories:[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, str(e)))
        return
    if type == "addon pack":
        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Are you sure you wish to create an Addon Pack?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]Cancel Backup[/COLOR][/B]", yeslabel="[B][COLOR springgreen]Create Pack[/COLOR][/B]"):
            ...
            try:
                zipf = zipfile.ZipFile(transPath(zipname), mode='w')
            except:
                try:
            ...
            log(selected)
            DP.create(ADDONTITLE,'[COLOR %s][B]Creating Zip File:[/B][/COLOR][CR]Please Wait[CR]' % COLOR2)
            if len(selected) > 0:
                added = []
                for item in selected:
                    added.append(addonfolds[item])
                    DP.update(0, "[COLOR %s][B]Creating Zip File:[/B][/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, addonfolds[item]))
                    for base, dirs, files in os.walk(os.path.join(ADDONS,addonfolds[item])):
                        files[:] = [f for f in files if f not in exclude_files]
                        for file in files:
                            if file.endswith('.pyo'): continue
                            DP.update(0, "[COLOR %s][B]Creating Zip File:[/B][/COLOR][CR][COLOR %s]%s[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, addonfolds[item], COLOR1, file))
                    ...
                    if os.path.exists(dep):
                        ...
                        for depends in match:
                            ...
                            DP.update(0, "[COLOR %s][B]Creating Zip File:[/B][/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, depends))
                            for base, dirs, files in os.walk(os.path.join(ADDONS,depends)):
                                files[:] = [f for f in files if f not in exclude_files]
                                for file in files:
                                    if file.endswith('.pyo'): continue
                                    DP.update(0, "[COLOR %s][B]Creating Zip File:[/B][/COLOR][CR][COLOR %s]%s[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, depends, COLOR1, file))
                                    fn = os.path.join(base, file)
                                    zipf.write(fn, fn[len(ADDONS):], zipfile.ZIP_DEFLATED)
                                    added.append(depends)
            DIALOG.ok(ADDONTITLE, "[COLOR %s]%s[/COLOR] [COLOR %s]backup successful:[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR1, name, COLOR2, COLOR1, zipname))
    elif type == "build":
        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Are you sure you wish to backup the current build?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]Cancel Backup[/COLOR][/B]", yeslabel="[B][COLOR springgreen]Backup Build[/COLOR][/B]"):
            ...
            for_progress  = 0
            ITEM          = []
            if not DIALOG.yesno (ADDONTITLE, "[COLOR% s] Do you want the addon_data folder to be included? This includes [COLOR% s] ALL [/COLOR] add-on settings including passwords but may also contain useful data such as skin shortcuts We recommend [COLOR% s] CHECKED [/COLOR] to remove unneeded add-on data folders. [COLOR% s]% s [/COLOR] data folder excluded [/COLOR] "% (COLOR2, COLOR1, COLOR1, COLOR1, ADDON_ID), yeslabel = '[B] [COLOR springgreen] Add addon_data [/COLOR] [/B]', nolabel = '[B] [COLOR red] No addon_data [/COLOR] [/B] '):
            ...
            try:
                ...
                try:
                    ...
                    if DIALOG.yesno(ADDONTITLE, "[COLOR %s]We are unable to write to the current backup directory, would you like to change the location?[/COLOR]" % COLOR2, yeslabel="[B][COLOR springgreen]Change Directory[/COLOR][/B]", nolabel="[B][COLOR red]Cancel[/COLOR][/B]"):
                        ...
                        return
            DP.create("[COLOR %s]%s[/COLOR][COLOR %s]: Creating Zip[/COLOR]" % (COLOR1, ADDONTITLE,COLOR2), "[COLOR %s]Creating back up zip Please Wait...[/COLOR]" % COLOR2)
            for base, dirs, files in os.walk(HOME):
                dirs[:] = [d for d in dirs if d not in exclude_dirs]
                files[:] = [f for f in files if f not in exclude_files]
                for file in files:
            ...
            for base, dirs, files in os.walk(HOME):
                ...
                for file in files:
                    try:
                        ...
                        DP.update(int(progress), '[COLOR %s]Creating back up zip: [COLOR%s]%s[/COLOR] / [COLOR%s]%s[/COLOR][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, for_progress, COLOR1, N_ITEM, COLOR1, file))
                        fn = os.path.join(base, file)
                        if file in LOGFILES: log("[Back Up] Type = '%s': Ignore %s" % (type, file), LOGNOTICE); continue
                        elif os.path.join(base, file) in bad_files: log("[Back Up] Type = '%s': Ignore %s" % (type, file), LOGNOTICE); continue
                        elif os.path.join('addons', 'packages') in fn: log("[Back Up] Type = '%s': Ignore %s" % (type, file), LOGNOTICE); continue
                        elif os.path.join(ADDONS, 'inputstream.adaptive') in fn: log("[Back Up] Type = '%s': Ignore %s" % (type, file), LOGNOTICE); continue
                        elif file.endswith('.csv'): log("[Back Up] Type = '%s': Ignore %s" % (type, file), LOGNOTICE); continue
                            ...
                            if temp in ['Addons', 'ADSP', 'Epg', 'MyMusic', 'MyVideos', 'Textures', 'TV', 'ViewModes']:
                                if not file == latestDB(temp):  log("[Back Up] Type = '%s': Ignore %s" % (type, file), LOGNOTICE); continue
                        try:
                            zipf.write(fn, fn[len(HOME):], zipfile.ZIP_DEFLATED)
                            extractsize += os.path.getsize(fn)
                        except Exception as e:
                            log("[Back Up] Type = '%s': Unable to backup %s" % (type, file), LOGNOTICE)
                            log("%s / %s" % (Exception, e))
                        if DP.iscanceled():
                            DP.close()
                        ...
                        log("[Back Up] Type = '%s': Unable to backup %s" % (type, file), LOGNOTICE)
                        log("Build Backup Error: %s" % str(e), LOGNOTICE)
            if 'addon_data' in exclude_dirs:
                ...
                for fold in match:
                    ...
                    if not fd in ['skin.confluence', 'skin.re-touch', 'skin.estuary', 'skin.estouchy']:
                        ...
                        if os.path.exists(xml):
                            source   = xbmcvfs.File(xml); link = source.read(); source.close();
            ...
            with xbmcvfs.File(info, 'w') as f:
                f.write('name="%s"\n' % name)
                f.write('extracted="%s"\n' % extractsize)
                f.write('zipsize="%s"\n' % os.path.getsize(transPath(zipname)))
                f.write('skin="%s"\n' % currSkin())
                f.write('created="%s"\n' % datetime.now().date())
            ...
            DIALOG.ok(ADDONTITLE, "[COLOR %s]%s[/COLOR] [COLOR %s]backup successful:[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR1, name, COLOR2, COLOR1, zipname))
    elif type == "guifix":
        if name == "":
        ...
        guizipname = transPath(os.path.join(mybuilds, '%s_guisettings.zip' % guiname))
        if os.path.exists(GUISETTINGS):
            ...
            try:
                ...
                for fold in match:
                    fd = os.path.split(fold[:-1])[1]
                    if not fd in ['skin.confluence', 'skin.re-touch', 'skin.estuary', 'skin.estouchy']:
                        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to add the following skin folder to the GuiFix Zip File?[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, fd), yeslabel="[B][COLOR springgreen]Add Skin[/COLOR][/B]", nolabel="[B][COLOR red]Skip Skin[/COLOR][/B]"):
                            for base, dirs, files in os.walk(os.path.join(ADDOND,fold)):
                                files[:] = [f for f in files if f not in exclude_files]
                                for file in files:
                                    fn = os.path.join(base, file)
                                ...
                                if 'script.skinshortcuts' in matchxml:
                                    for base, dirs, files in os.walk(os.path.join(ADDOND,'script.skinshortcuts')):
                                        ...
                                        for file in files:
                                            fn = os.path.join(base, file)
                                            zipf.write(fn, fn[len(USERDATA):], zipfile.ZIP_DEFLATED)
                        else: log("[Back Up] Type = '%s': %s ignored" % (type, fold), LOGNOTICE)
            except Exception as e:
                log("[Back Up] Type = '%s': %s" % (type, e), LOGNOTICE)
                pass
            zipf.close()
        ...
        else: log("[Back Up] Type = '%s': guisettings.xml not found" % type, LOGNOTICE)
        if name == "":
            DIALOG.ok(ADDONTITLE, "[COLOR %s]GuiFix backup successful:[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, guizipname))
    elif type == "theme":
        ...
        try:
            if not SKIN in ['skin.confluence', 'skin.estuary']:
                ...
                if len(match2) > 1:
                    if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Would you like to go through the Texture Files for?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, SKIN), yeslabel="[B][COLOR springgreen]Add Textures[/COLOR][/B]", nolabel="[B][COLOR red]Skip Textures[/COLOR][/B]"):
                        ...
                        match2 = glob.glob(os.path.join(skinfold,'*.xbt'))
                        for xbt in match2:
                            if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Would you like to add the Texture File [COLOR %s]%s[/COLOR]?[CR]from [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR1, COLOR2, xbt.replace(skinfold, "")[1:], COLOR1, SKIN), yeslabel="[B][COLOR springgreen]Add Textures[/COLOR][/B]", nolabel="[B][COLOR red]Skip Textures[/COLOR][/B]"):
                                fn  = xbt
                                fn2 = fn.replace(HOME, "")
                                zipf.write(fn, fn2, zipfile.ZIP_DEFLATED)
                else:
                    for xbt in match2:
                        if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Would you like to add the Texture File [COLOR %s]%s[/COLOR]?[CR]from [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR2, COLOR1, xbt.replace(skinfold, "")[1:], COLOR1, SKIN), yeslabel="[B][COLOR springgreen]Add Textures[/COLOR][/B]", nolabel="[B][COLOR red]Skip Textures[/COLOR][/B]"):
                            fn  = xbt
                            fn2 = fn.replace(HOME, "")
                            zipf.write(fn, fn2, zipfile.ZIP_DEFLATED)
                ad_skin = os.path.join(ADDOND, SKIN, 'settings.xml')
                if os.path.exists(ad_skin):
                    if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, ADDONTITLE, COLOR2), "[COLOR %s]Would you like to go add the [COLOR %s]settings.xml[/COLOR] in [COLOR %s]/addon_data/[/COLOR] for?[CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, COLOR1, COLOR1, SKIN), yeslabel="[B][COLOR springgreen]Add Settings[/COLOR][/B]", nolabel="[B][COLOR red]Skip Settings[/COLOR][/B]"):
                        skinfold = os.path.join(ADDOND, SKIN)
                        zipf.write(ad_skin, ad_skin.replace(HOME, ""), zipfile.ZIP_DEFLATED)
                f = xbmcvfs.File(transPath(os.path.join(ADDONS, SKIN, 'addon.xml'))); r = f.read(); f.close()
                    ...
                    for base, dirs, files in os.walk(fn):
                        ...
                        for file in files:
                            try:
                                ...
                                log("[Back Up] Type = '%s': Unable to backup %s" % (type, file), LOGNOTICE)
                                log("Backup Error: %s" % str(e), LOGNOTICE)
            ...
            zipf.close()
            log("[Back Up] Type = '%s': %s" % (type, str(e)), LOGNOTICE)
            DIALOG.ok(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] theme zip failed:[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR1, themename, COLOR2, COLOR1, str(e)))
            if not tempzipname == '':
                try: xbmcvfs.delete(transPath(tempzipname))
                except Exception as e: log(str(e))
            else:
                try: xbmcvfs.delete(transPath(zipname))
                except Exception as e: log(str(e))
            return
        zipf.close()
        if not tempzipname == '':
            ...
            if success == 0:
                xbmcvfs.copy(tempzipname, zipname)
                xbmcvfs.delete(tempzipname)
        DIALOG.ok(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] theme zip successful:[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR1, themename, COLOR2, COLOR1, zipname))
    elif type == "addondata":
        if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Are you sure you wish to backup the current addon_data?[/COLOR]" % COLOR2, nolabel="[B][COLOR red]Cancel Backup[/COLOR][/B]", yeslabel="[B][COLOR springgreen]Backup Addon_Data[/COLOR][/B]"):
            ...
            DP.create("[COLOR %s]%s[/COLOR][COLOR %s]: Creating Zip[/COLOR]" % (COLOR1, ADDONTITLE,COLOR2), "[COLOR %s]Creating back up zip[CR]Please Wait...[/COLOR][CR]" % COLOR2)
            ...
            for base, dirs, files in os.walk(ADDOND):
                ...
                for file in files:
                    try:
                        for_progress += 1
                        progress = percentage(for_progress, N_ITEM)
                        DP.update(int(progress), '[COLOR %s]Creating back up zip: [COLOR%s]%s[/COLOR] / [COLOR%s]%s[/COLOR][CR][COLOR %s]%s[/COLOR][CR]' % (COLOR2, COLOR1, for_progress, COLOR1, N_ITEM, COLOR1, file))
                        fn = os.path.join(base, file)
                        if file in LOGFILES: log("[Back Up] Type = '%s': Ignore %s" % (type, file), LOGNOTICE); continue
                        elif os.path.join(base, file) in bad_files: log("[Back Up] Type = '%s': Ignore %s" % (type, file), LOGNOTICE); continue
                        elif os.path.join('addons', 'packages') in fn: log("[Back Up] Type = '%s': Ignore %s" % (type, file), LOGNOTICE); continue
...
def restoreLocal(type):
    ...
    log("[RESTORE BACKUP %s] File: %s " % (type.upper(), file), LOGNOTICE)
    if file == "" or not file.endswith('.zip'):
        LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Local Restore: Cancelled[/COLOR]" % COLOR2)
        return
    DP.create(ADDONTITLE,'[COLOR %s]Installing Local Backup Please Wait[/COLOR]' % COLOR2)
    ...
    log("Restoring to %s" % loc, LOGNOTICE)
    display = os.path.split(file)
    fn = display[1]
    try:
        zipfile.ZipFile(file,  'r')
    except:
        DP.update(0, '[COLOR %s]Unable to read zipfile from current location.[/COLOR][CR]Copying file to packages' % COLOR2)
        pack = os.path.join('special://home', 'addons', 'packages', fn)
        xbmcvfs.copy(file, pack)
        file = transPath(pack)
        DP.update(0, '[CR]Copying file to packages: Complete')
        zipfile.ZipFile(file, 'r')
    ...
    if int(errors) >= 1:
        yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, fn), 'Completed: [COLOR %s]%s%s[/COLOR] [Errors:[COLOR %s]%s[/COLOR]][CR]Would you like to view the errors?[/COLOR]' % (COLOR1, percent, '%', COLOR1, errors), nolabel='[B][COLOR red]No Thanks[/COLOR][/B]',yeslabel='[B][COLOR springgreen]View Errors[/COLOR][/B]')
        if yes:
...
def restoreExternal(type):
    ...
    try:
        ...
        log("Not a working url, if source was local then use local restore option", LOGNOTICE)
        log("External Source: %s" % source, LOGNOTICE)
        return
    log("[RESTORE EXT BACKUP %s] File: %s " % (type.upper(), source), LOGNOTICE)
    zipit = os.path.split(source); zname = zipit[1]
    DP.create(ADDONTITLE,'[COLOR %s]Downloading Zip file Please Wait[/COLOR]' % COLOR2)
    if type == "gui": loc = USERDATA
    ...
    downloader.download(source, file, DP, 'Zip File')
    DP.update(0,'Installing External Backup[CR][CR]Please Wait') 
    percent, errors, error = extract.all(file,loc,DP)
    fixmetas()
    clearS('build')
    DP.close()
    ...
    lookandFeelData('save')
    if int(errors) >= 1:
        yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR][CR]Completed: [COLOR %s]%s%s[/COLOR] [Errors:[COLOR %s]%s[/COLOR]][CR]Would you like to view the errors?[/COLOR]' % (COLOR2, COLOR1, zname, COLOR1, percent, '%', COLOR1, errors), nolabel='[B][COLOR red]No Thanks[/COLOR][/B]',yeslabel='[B][COLOR springgreen]View Errors[/COLOR][/B]')
        if yes:
            TextBox(ADDONTITLE, error.replace('\t',''))
    setS('installed', 'true')
    setS('extract', str(percent))
...
def Grab_Log(file=False, old=False, wizard=False):
    ...
    if len(logsfound) > 0:
        ...
        if file == True: return logsfound[-1]
            ...
            filename    = xbmcvfs.File(logsfound[-1])
            logtext     = filename.read()
            filename.close()
            return logtext
    else:
...
def whiteList(do):
    ...
    if   do == 'edit':
        ...
        for item in current:
            ...
            idList.append(id)
        lines = a.split('\n')
        with xbmcvfs.File(WHITELIST, 'w') as f:
            for item in lines:
                try:
        ...
        try:
            ...
            DIALOG.ok(ADDONTITLE, "[COLOR %s]Whitelist has been exported to:[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, os.path.join(source, 'whitelist.txt')))
        ...
        if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Are you sure you want to clear your whitelist?[CR]This process can't be undone.[/COLOR]" % COLOR2, yeslabel="[B][COLOR springgreen]Yes Remove[/COLOR][/B]", nolabel="[B][COLOR red]No Cancel[/COLOR][/B]"):
            LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Clear Whitelist Cancelled[/COLOR]" % (COLOR2))
            return
        try:
            xbmcvfs.delete(WHITELIST)
        ...
        try:
            for root, dirs, files in os.walk(PACKAGES):
                ...
                if file_count > 0:
                    ...
                    if over: yes=1
                    else: yes=DIALOG.yesno("[COLOR %s]Delete Package Files[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR] files found / [COLOR %s]%s[/COLOR] in size.[CR]Do you want to delete them?" % (COLOR1, str(file_count), COLOR1, size), nolabel='[B][COLOR red]Don\'t Clear[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Clear Packages[/COLOR][/B]')
                    if yes:
...
def fList():
    f = []
    if not workingURL(APKFILE) == True: return f
    try:
        a = openURL(APKFILE).decode('utf-8').replace('\n','').replace('\r','').replace('\t','')
        match = re.compile('apkfile="(.+?)"').findall(a)
        match2 = re.compile('ignore="(.+?)"').findall(a)
        match3 = re.compile('nignore="(.+?)"').findall(a)
        if len(match) > 0:
            if (len(match2) + len(match3)) > 0:
                if any(os.path.exists(transPath(i)) for i in match2) and not any(os.path.exists(transPath(i)) for i in match3):
                    for item in match:
                        item = transPath(item)
                        if not os.path.exists(transPath(item)): continue
                        f.append(item)
            else:
                for item in match:
                    item = transPath(item)
...
def clearCache(over=None):
    ...
    dbfiles   = [
        ...
        (os.path.join(ADDOND, 'plugin.video.gaia', 'meta.db')),
        (os.path.join(ADDOND, 'plugin.video.thecrew', 'cache.db')),
        (os.path.join(ADDOND, 'plugin.video.thecrew', 'meta.5.db')),
        (os.path.join(ADDOND, 'plugin.video.thecrew', 'cache.providers.13.db')),
        (os.path.join(ADDOND, 'plugin.video.homelander', 'cache.db')),
        (os.path.join(ADDOND, 'plugin.video.homelander', 'cache.meta.5.db')),
        (os.path.join(ADDOND, 'plugin.video.homelander', 'cache.providers.13.db')),
                ...
                if file_count > 0:
                    for f in files:
                        if not f in LOGFILES:
                            try:
                                os.unlink(os.path.join(root, f))
                                log("[Wiped] %s" % os.path.join(root, f), LOGNOTICE)
                                delfiles += 1
                            except:
                                pass
                        else: log('Ignore Log File: %s' % f, LOGNOTICE)
                    for d in dirs:
                        try:
                            shutil.rmtree(os.path.join(root, d))
                            delfiles += 1
                            log("[Success] cleared %s files from %s" % (str(file_count), os.path.join(item,d)), LOGNOTICE)
                        except:
                            log("[Failed] to wipe cache in: %s" % os.path.join(item,d), LOGNOTICE)
        else:
            for root, dirs, files in os.walk(item):
                dirs[:] = [d for d in dirs if d not in excludes]
                for d in dirs:
                    if not str(d.lower()).find('cache') == -1:
                        try:
                            shutil.rmtree(os.path.join(root, d))
                            delfiles += 1
                            log("[Success] wiped %s " % os.path.join(root,d), LOGNOTICE)
                        except:
                            log("[Failed] to wipe cache in: %s" % os.path.join(item,d), LOGNOTICE)
        ...
        if len(files) > 0:
            for item in files:
                if os.path.exists(item):
                    delfiles += 1
                    try:
                        textdb = database.connect(item, isolation_level=None)
                        textexe = textdb.cursor()
                    except Exception as e:
                    ...
                    if 'Database' in item:
                        try:
                            ...
                            textdb.commit()
                            textexe.close()
                            log("[Success] wiped %s" % item, LOGNOTICE)
                        except Exception as e:
                            log("[Failed] wiped %s: %s" % (item, str(e)), LOGNOTICE)
                    else:
                        ...
                        for table in textexe.fetchall():
                            try:
                                ...
                                textexe.execute("VACUUM")
                                textdb.commit()
                                log("[Success] wiped %s in %s" % (table[0], item), LOGNOTICE)
                            except Exception as e:
                                try:
                                    log("[Failed] wiped %s in %s: %s" % (table[0], item, str(e)), LOGNOTICE)
                                except:
                                    pass
                        textexe.close()
        else: log("Clear Cache: Clear Video Cache Not Enabled", LOGNOTICE)
...
def checkSources():
    ...
    if len(match) > 0:
        ...
        for name, path, sharing in match2:
            ...
            DP.update(perc, '[CR][COLOR %s]Checking [COLOR %s]%s[/COLOR]:[/COLOR][CR][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name, COLOR1, path))
            if 'http' in path:
                working = workingURL(path)
                if not working == True:
                    bad.append([name, path, sharing, working])

        log("Bad Sources: %s" % len(bad), LOGNOTICE)
        if len(bad) > 0:
            choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] Source(s) have been found Broken[CR]Would you like to Remove all or choose one by one?[/COLOR]" % (COLOR1, len(bad), COLOR2), yeslabel="[B][COLOR springgreen]Remove All[/COLOR][/B]", nolabel="[B][COLOR red]Choose to Delete[/COLOR][/B]")
            if choice == 1:
                remove = bad
            else:
                for name, path, sharing, working in bad:
                    log("%s sources: %s, %s" % (name, path, working), LOGNOTICE)
                    if DIALOG.yesno(ADDONTITLE, "[COLOR %s]%s[/COLOR][COLOR %s] was reported as non working[COLOR %s]%s[/COLOR][COLOR %s]%s[/COLOR]" % (COLOR1, name, COLOR2, COLOR1, path, COLOR1, working), yeslabel="[B][COLOR springgreen]Remove Source[/COLOR][/B]", nolabel="[B][COLOR red]Keep Source[/COLOR][/B]"):
                        remove.append([name, path, sharing, working])
                        log("Removing Source %s" % name, LOGNOTICE)
                    else: log("Source %s was not removed" % name, LOGNOTICE)
            if len(remove) > 0:
                for name, path, sharing, working in remove:
                    a = a.replace('\n        <source>\n            <name>%s</name>\n            <path pathversion="1">%s</path>\n            <allowsharing>%s</allowsharing>\n        </source>' % (name, path, sharing), '')
                    log("Removing Source %s" % name, LOGNOTICE)

                f = xbmcvfs.File(SOURCES, 'w')
                f.write(str(a))
                f.close()
                alive = len(match) - len(bad)
                kept = len(bad) - len(remove)
                removed = len(remove)
                DIALOG.ok(ADDONTITLE, "[COLOR %s]Checking sources for broken paths has been completed[CR]Working: [COLOR %s]%s[/COLOR] | Kept: [COLOR %s]%s[/COLOR] | Removed: [COLOR %s]%s[/COLOR][/COLOR]" % (COLOR2, COLOR2, COLOR1, alive, COLOR1, kept, COLOR1, removed))
            else: log("No Bad Sources to be removed.", LOGNOTICE)
        else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]All Sources Are Working[/COLOR]" % COLOR2)
    else: log("No Sources Found", LOGNOTICE)
...
def checkRepos():
    ...
    while start < sleeptime:
        ...
        DP.update(perc, '[CR][COLOR %s]Checking: [/COLOR][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, repolist[start-1].replace(ADDONS, '')[1:]))
        xbmc.sleep(1000)
    if DP.iscanceled():
        DP.close()
    ...
    for item in fails:
        log("Bad Repository: %s " % item, LOGNOTICE)
...
def killxbmc(over=None):
    if over:
        ...
        choice = DIALOG.yesno('Force Close Kodi', '[COLOR %s]You are about to close Kodi[CR]Would you like to continue?[/COLOR]' % COLOR2, nolabel='[B][COLOR white] No Cancel[/COLOR][/B]',yeslabel='[B][COLOR white]Force Close Kodi[/COLOR][/B]')
    if choice == 1:
        log("Force Closing Kodi: Platform[%s]" % str(platform()), LOGNOTICE)
        os._exit(1)
...
def reloadFix(default=None):
    DIALOG.ok(ADDONTITLE, "[COLOR %s]WARNING: Sometimes Reloading the Profile causes Kodi to crash.[CR]  While Kodi is Reloading the Profile Please Do Not Press Any Buttons![/COLOR]" % COLOR2)
    if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
    if default == None:
        lookandFeelData('save')
    redoThumbs()
...
def kodi19Fix():
    dbfile = latestDB('Addons')
    dbfile = os.path.join(DATABASE, dbfile)
    conn = database.connect(dbfile)
    addonfolder = transPath(os.path.join('special://home', 'addons'))
...
def restoreAutoUpdates(table='update_rules'):
    dbfile = latestDB('Addons')
    dbfile = os.path.join(DATABASE, dbfile)
    ...
    if table == 'update_rules':
        try:
            try:
                textexe.execute("DELETE FROM update_rules")
                LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Addons Auto Updates Restored[/COLOR]" % COLOR2)
            except:
                textexe.execute("DROP TABLE IF EXISTS update_rules")
                textexe.execute("CREATE TABLE IF NOT EXISTS update_rules (id integer primary key, addonID TEXT, updateRule INTEGER);")
                textexe.execute("CREATE UNIQUE INDEX idxUpdate_rules ON update_rules(addonID, updateRule);")
                LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Addons Auto Updates Restored[/COLOR]" % COLOR2)
        except:
            LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Error Restoring Auto updates[/COLOR]" % COLOR2)
        textdb.commit()
        textexe.close()
    else:
        try:
            textexe.execute("DELETE FROM %s" % table)
        except:
            pass
        textdb.commit()
        textexe.close()
...
def purgeDb(name):
    ...
    log('Purging DB %s.' % name, LOGNOTICE)
    ...
    textexe.close()
    log('%s DB Purging Complete.' % name, LOGNOTICE)
...
def oldThumbs():
    ...
    log("%s total thumbs cleaned up." % str(len(images)), LOGNOTICE)
    for id in ids:
...
def SFxmls():
    skinsh = os.path.join(ADDOND, 'script.skinshortcuts/')
    superf = os.path.join(ADDONS, 'plugin.program.super.favourites/')
    simplef = os.path.join(ADDONS, 'plugin.program.simple.favourites/')

    skin_xml = xbmcgui.Dialog().browse(1, 'Select skinshortcut submenu xml file', 'files', mask='.xml', useThumbs=False, treatAsFolder=False,
                                       defaultt=skinsh, enableMultiple=False)
    FAV_list = []
    if skin_xml:
        with open(skin_xml, 'rb') as xml:
            infos = xml.read()
            ...
            for shortcut in shortcuts:
                ...
                newsf = '<favourite name="{}" thumb="{}" fanart="none">{}</favourite>'.\
                    format(label.encode('utf-8'), thumb, action)
                xbmc.log('FAVOURITE: %s' % newsf)
                FAV_list.append(newsf)
    else:
        return
    ret = xbmcgui.Dialog().select('Select Addon', ['Simple Fav', 'Super Fav'])

    if ret == -1:
        return
    elif ret == 0:
        sf_xml = xbmcgui.Dialog().browse(1, 'Select SimpleFavourite xml file', 'files', mask='.xml', useThumbs=False, treatAsFolder=False,
                                         defaultt=simplef, enableMultiple=False)
    elif ret == 1:
        sf_xml = xbmcgui.Dialog().browse(1, 'Select SuperFavourite xml file', 'files', mask='.xml', useThumbs=False, treatAsFolder=False,
                                         defaultt=superf, enableMultiple=False)
    else:
        return
    ...
    xbmcgui.Dialog().ok('SF XML CREATOR', 'NEW XML CREATED')


def emptyFilesFix(url, over=False):
    total = fileCount(url); start = 0
    DP.create(ADDONTITLE, "[COLOR %s]Fix empty __init__.py files[CR][CR]Please Wait[/COLOR]" % COLOR2)
    for root, dirs, files in os.walk(url):
        for file in files:
            start += 1
            perc = int(percentage(start, total))
            if file == '__init__.py':
                ...
                if os.stat(os.path.join(root, file)).st_size==0:
                    ...
                    if DP.iscanceled():
                        DP.close()
                        LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Fix empty __init__.py files Cancelled[/COLOR]" % COLOR2)
                        sys.exit()
    DP.close()
    log("[Fix empty __init__.py files] Complete", lognot)
    if over == False: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Fix empty __init__.py files: Complete![/COLOR]" % COLOR2)