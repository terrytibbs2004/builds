...
addonObject = addon('script.module.yourscrapers')
...
def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('yourscrapers_settings'))
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		homeWindow.setProperty('yourscrapers_settings', jsdumps(settings_dict))
		return settings_dict
	except:
		return None

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear yourscrapers.log if setting to reverse has been changed
	if homeWindow.getProperty('yourscrapers.debug.reversed') != setting('debug.reversed'):
		homeWindow.setProperty('yourscrapers.debug.reversed', setting('debug.reversed'))
		execute('RunPlugin(plugin://script.module.yourscrapers/?action=tools_clearLogFile)')
...
def isVersionUpdate():
	...
	try:
		...
		xbmc.log('YourScrapers Addon Data Path Does not Exist. Creating Folder....', LOGINFO)
		addon_folder = transPath('special://profile/addon_data/script.module.yourscrapers')
		xbmcvfs.mkdirs(addon_folder)
	try:
		with open(versionFile, 'r') as fh: oldVersion = fh.read()
	except: oldVersion = '0'
	try:
		curVersion = addon('script.module.yourscrapers').getAddonInfo('version')
...
def clean_settings():
	def _make_content(dict_object):
		...
		content += '\n</settings>'
		return content
	addon_id = 'script.module.yourscrapers'
	try:
		removed_settings = []
		active_settings = []
