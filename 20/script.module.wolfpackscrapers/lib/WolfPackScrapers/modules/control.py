...
addonObject = addon('script.module.wolfpackscrapers')
...
def autoTraktSubscription(tvshowtitle, year, imdb, tvdb):
    from WolfPackScrapers.modules import libtools
...
def artPath():
    theme = appearance()
    return os.path.join(xbmcaddon.Addon('plugin.video.wolfpack').getAddonInfo('path'), 'resources', 'media', theme)
...
def artwork():
    execute('RunPlugin(plugin://plugin.video.wolfpack)')
...
def metaFile():
    if condVisibility('System.HasAddon(plugin.video.wolfpack)'):
        return os.path.join(xbmcaddon.Addon('plugin.video.wolfpack').getAddonInfo('path'), 'resources', 'data', 'meta.db')
