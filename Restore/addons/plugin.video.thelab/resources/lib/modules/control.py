...
def artPath():
    theme = appearance()
    #if theme in ['-', '']: return
    #elif condVisibility('System.HasAddon(script.thelab.artwork)'):
#        return os.path.join(xbmcaddon.Addon('script.thelab.artwork').getAddonInfo('path'), 'resources', 'media', theme)
    return os.path.join(xbmcaddon.Addon('plugin.video.thelab').getAddonInfo('path'), 'resources', 'media', theme)

def appearance():
    appearance = setting('appearance.1').lower()
    #appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.thelab.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://plugin.video.thelab)')
...
# def metaFile():
    # if condVisibility('System.HasAddon(script.thelab.metadata)'):
        # return os.path.join(xbmcaddon.Addon('script.thelab.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
...
def clean_settings(info=True):#Fen code
    ...
    def _make_content(dict_object):
        ...
        return content
    try:
        for addon_id in ('plugin.video.thelab', 'script.module.thelabscrapers'):
            removed_settings = []
            active_settings = []
