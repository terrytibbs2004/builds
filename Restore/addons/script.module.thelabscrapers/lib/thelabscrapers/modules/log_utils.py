...
DEBUGPREFIX = '[ TheLabScrapers {0} | DEBUG ]'.format(version)
INFOPREFIX = '[ TheLabScrapers | INFO ]'
LOGPATH = control.transPath('special://logpath/')
log_file = os.path.join(LOGPATH, 'thelab.log')
debug_enabled = control.addon('plugin.video.thelab').getSetting('addon.debug')
...
def log(msg, trace=0):
    ...
    try:
        ...
        try:
            xbmc.log('TheLabScrapers Logging Failure: %s' % e, LOGDEBUG)
        except:
            pass
