...
def get_movies(url,local=False,reco=0,global_s=False,return_results=False):
        ...
        for items in all_years:
            ...
            if 0:
               if  name not in all_n:
                ...
                aa=addDir3('[COLOR goldenrod]'+items+'[/COLOR]',url,14,'http://narcacist.com/images/asg/icon.png','http://narcacist.com/images/asg/fanart.jpg',items,collect_all=True)
                all_d.append(aa)
            else:
                aa=addDir3('[COLOR goldenrod]'+items+'[/COLOR]',url,14,'http://narcacist.com/images/asg/icon.png','http://narcacist.com/images/asg/fanart.jpg',items,collect_all=True)
                all_d.append(aa)
   ...
   if   max_page==-1 and not return_results:
        if not global_s:
            xbmcgui.Dialog().ok(Addon.getAddonInfo('name'),'[COLOR goldenrod][I]%s[/I][/COLOR]'%Addon.getLocalizedString(32183))
   ...
   for data in html['seasons']:
     ...
     try:
        if 'air_date' in data:
               ...
               datea='[COLOR goldenrod]'+str(time.strptime(data['air_date'], '%Y-%m-%d'))+'[/COLOR]\n'
               ...
               if a>b:
                 ...
                 color='goldenrod'
        datea='[COLOR goldenrod]'+txt_1+time.strftime( "%d-%m-%Y",a) + '[/COLOR]\n'
     except Exception as e:
     ...
     try:
        ...
        if 'air_date' in data:
               ...
               if a>b:
                 ...
                 color2='goldenrod'
        datea='[COLOR goldenrod]'+Addon.getLocalizedString(32187)+time.strftime( "%d-%m-%Y",a) + '[/COLOR]\n'
     except:
             try:
                datea=data['air_date']
             except:
