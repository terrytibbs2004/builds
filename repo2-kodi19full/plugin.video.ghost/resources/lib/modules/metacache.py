...
def fetch(items, lang='en', user=''):
    ...
    for i in range(0, len(items)):
        try:
            dbcur.execute("SELECT * FROM meta WHERE (imdb = '%s' and lang = '%s' and user = '%s' and not imdb = '0') or (tmdb = '%s' and lang = '%s' and user = '%s' and not tmdb = '0')" % (items[i]['imdb'], lang, user, items[i]['tmdb'], lang, user))
    ...
    try:
        ...
        for m in meta:
            try:
                ...
                i = repr(m['item'])
                try:
                    dbcur.execute("DELETE * FROM meta WHERE (imdb = '%s' and lang = '%s' and user = '%s' and not imdb = '0') or (tvdb = '%s' and lang = '%s' and user = '%s' and not tvdb = '0' or (tmdb = '%s' and lang = '%s' and user = '%s' and not tmdb = '0'))" % (m['imdb'], m['lang'], m['user'], m['tvdb'], m['lang'], m['user'], m['tmdb'], m['lang'], m['user']))
                except:
                    pass
