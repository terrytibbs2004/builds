...
INCLUDEATREDIES    = wiz.getS('includehomelander')
INCLUDEFILMNET    = wiz.getS('includemoria')
...
def buildMenu():
    ...
    if bf == False:
        ...
        return
    total, count15, count16, count17, count18, count19, count20, adultcount, hidden = wiz.buildCount()
    third = False; addin = []
    if THIRDPARTY == 'true':
    ...
    if len(match) >= 1:
        if SEPERATE == 'true':
            ...
            if count20 > 0:
                ...
                if SHOW20 == 'true':
                    addFile('[B]%s Nexus Builds(%s)[/B]' % (state, count20), '', icon=ICONBUILDS, themeit=THEME3)
                    for name, version, url, gui, kodi, theme, icon, fanart, adult, description, in match:
                        if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
                        if not DEVELOPER == 'true' and wiz.strTest(name): continue
            ...
            if count19 > 0:
                ...
                if SHOW19 == 'true':
                    addFile('[B]%s Matrix Builds(%s)[/B]' % (state, count19), '', icon=ICONBUILDS, themeit=THEME3)
                    for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
                        if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
                        if not DEVELOPER == 'true' and wiz.strTest(name): continue
                        kodiv = int(float(kodi))
            ...
            if count18 > 0:
                ...
                if SHOW18 == 'true':
                    addFile('[B]%s Leia Builds(%s)[/B]' % (state, count18), '', icon=ICONBUILDS, themeit=THEME3)
                    for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
                        if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
                        if not DEVELOPER == 'true' and wiz.strTest(name): continue
                        kodiv = int(float(kodi))
            ...
            if count17 > 0:
                ...
                if SHOW17 == 'true':
                    addFile('[B]%s Krypton Builds(%s)[/B]' % (state, count17), '', icon=ICONBUILDS, themeit=THEME3)
                    for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
                        if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
                        if not DEVELOPER == 'true' and wiz.strTest(name): continue
                        kodiv = int(float(kodi))
                        if kodiv == 17:
                            menu = createMenu('install', '', name)
                            addDir('[%s] [COLOR %s][B] %s [/B][/COLOR](v%s)' % (float(kodi), name, version), 'viewbuild', name, description=description, fanart=fanart,icon=icon, menu=menu, themeit=THEME2)
            if count16 > 0:
                state = '+' if SHOW16 == 'false' else '-'
                # addFile('[B]%s Jarvis Builds(%s)[/B]' % (state, count16), 'togglesetting',  'show16', themeit=THEME3)
                if SHOW16 == 'true':
                    addFile('[B]%s Jarvis Builds(%s)[/B]' % (state, count16), '', icon=ICONBUILDS, themeit=THEME3)
                    for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
                        if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
                        if not DEVELOPER == 'true' and wiz.strTest(name): continue
                        kodiv = int(float(kodi))
            ...
            if count15 > 0:
                ...
                if SHOW15 == 'true':
                    addFile('[B]%s Isengard and Below Builds(%s)[/B]' % (state, count15), '', icon=ICONBUILDS, themeit=THEME3)
                    for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
                        if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
                        if not DEVELOPER == 'true' and wiz.strTest(name): continue
                        kodiv = int(float(kodi))
    ...
    for version, url, minor, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
        ...
        if not themefile == 'http://':
            ...
            if not themecheck == False:
                ...
                link  = themecheck.decode('utf-8').replace('\n','').replace('\r','').replace('\t','')
...
def apkScraper(name=""):
    if name == 'kodi':
        ...
        kodiurl2 = 'https://mirrors.kodi.tv/releases/android/arm64-v8a/'
        url1 = wiz.openURL(kodiurl1).decode('utf-8').replace('\n', '').replace('\r', '').replace('\t', '')
        url2 = wiz.openURL(kodiurl2).decode('utf-8').replace('\n', '').replace('\r', '').replace('\t', '')
        x = 0
        ...
        for url, name, size, date in match1:
            ...
            try:
                ...
                if not url.find('_') == -1:
                    ...
                    name2 = tempname[2]
                    v2 = ''
                title = "[COLOR %s]%s v%s%s %s[/COLOR] [COLOR %s]%s[/COLOR] [COLOR %s]%s[/COLOR]" % (COLOR1, tempname[0].title(), tempname[1], v2.upper(), name2, COLOR2, size.replace(' ', '').replace('MiB', 'MB'), COLOR1, date)
                download = urljoin(kodiurl1, url)
                apkname = "%s v%s%s %s" % (tempname[0].title(), tempname[1], v2.upper(), name2)
                kodiapks.append([title, apkname, download])
                x += 1
            except:
        ...
        for url, name, size, date in match2:
            ...
            try:
                tempname = name.split('-')
                title = "[COLOR %s]%s v%s %s[/COLOR] [COLOR %s]%s[/COLOR] [COLOR %s]%s[/COLOR]" % (COLOR1, tempname[0].title(), tempname[1], tempname[2], COLOR2, size.replace(' ', '').replace('MiB', 'MB'), COLOR1, date)
                download = urljoin(kodiurl2, url)
                apkname = "%s v%s %s arm64" % (tempname[0].title(), tempname[1], tempname[2])
                kodiapks.append([title, apkname, download])
                x += 1
            except:
                wiz.log("Error on: %s" % name)
        kodiapks.sort(key=lambda x: x[0], reverse=True)
        for t, n, d in kodiapks:
            addFile(t, 'apkinstall', n, d)
...
def apkMenu(name=None, url=None):
    ...
    if not APKFILE == 'http://':
        ...
        if not TEMPAPKFILE == False:
            link = TEMPAPKFILE.decode('utf-8').replace('\n','').replace('\r','').replace('\t','')
...
def packInstaller(name, url):
    ...
    DP.create(ADDONTITLE,'[COLOR %s][B]Downloading:[/B][/COLOR] [COLOR %s]%s[/COLOR][COLOR %s Please Wait[/COLOR]' % (COLOR2, COLOR1, name, COLOR2))
    urlsplits = url.split('/')
    lib = legalFilename(os.path.join(PACKAGES, urlsplits[-1]))
    try: xbmcvfs.delete(lib)
    except: pass
    downloader.download(url, lib, DP, name)
    title = '[COLOR %s][B]Installing:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name)
    DP.update(0, '%s[COLOR %s]Please Wait[/COLOR]' % (title, COLOR2))
...
def skinInstaller(name, url):
    if not wiz.workingURL(url) == True: wiz.LogNotify("[COLOR %s]Addon Installer[/COLOR]" % COLOR1, '[COLOR %s]%s:[/COLOR] [COLOR %s]Invalid Zip Url![/COLOR]' % (COLOR1, name, COLOR2)); return
    if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
    DP.create(ADDONTITLE,'[COLOR %s][B]Downloading:[/B][/COLOR] [COLOR %s]%s[/COLOR][CR][CR][COLOR %s]Please Wait[/COLOR]' % (COLOR2, COLOR1, name, COLOR2))
    urlsplits = url.split('/')
    lib = legalFilename(os.path.join(PACKAGES, urlsplits[-1]))
    try: xbmcvfs.delete(lib)
    except: pass
...
def addonInstaller(plugin, url):
    if not ADDONFILE == 'http://':
        ...
        if ADDONWORKING == True:
            link = wiz.textCache(url).decode('utf-8').replace('\n','').replace('\r','').replace('\t','').replace('repository=""', 'repository="none"').replace('repositoryurl=""', 'repositoryurl="http://"').replace('repositoryxml=""', 'repositoryxml="http://"')
                ...
                for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
                    if os.path.exists(os.path.join(ADDONS, plugin)):
                        ...
                        if selected == 0:
                            ...
                            if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to remove the addon_data for:[CR][COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, plugin), yeslabel="[B][COLOR springgreen]Yes Remove[/COLOR][/B]", nolabel="[B][COLOR red]No Skip[/COLOR][/B]"):
...
def installAddon(name, url):
    ...
    DP.update(0, '%s[COLOR %s]Please Wait[/COLOR]' % (title, COLOR2))
    percent, errors, error = extract.all(lib,ADDONS,DP, title=title)
    DP.update(0, '%s[COLOR %s]Installing Dependencies[/COLOR]' % (title, COLOR2))
    installed(name)
    ...
    if os.path.exists(dep):
        ...
        for depends in match:
            if not 'xbmc.python' in depends:
                if not DP == None:
                    DP.update(0, '[CR][COLOR %s]%s[/COLOR]' % (COLOR1, depends))
                try:
...
def installed(addon):
    ...
    if os.path.exists(url):
        try:
            list  = xbmcvfs.File(url); g = list.read(); list.close()
...
def youtubeMenu(name=None, url=None):
    if not YOUTUBEFILE == 'http://':
        ...
        if not TEMPYOUTUBEFILE == False:
            link = TEMPYOUTUBEFILE.decode('utf-8').replace('\n','').replace('\r','').replace('\t','')
...
def maintMenu(view=None):
    ...
    if includeall == 'true':
        ...
        includegaia = 'true'
        includethecrew = 'true'
        includehomelander = 'true'
        includemoria = 'true'
    else:
        includethecrew = 'true' if INCLUDETHECREW     == 'true' else 'false'
        includehomelander = 'true' if INCLUDEHOMELANDER     == 'true' else 'false'
        includeMORIA = 'true' if INCLUDEMORIA     == 'true' else 'false' 
    ...
    if includevid == 'true':
        ...
        if xbmc.getCondVisibility('System.HasAddon(plugin.video.homelander)'): addFile('--- Include Homelander: %s' % includehomelander.replace('true',on).replace('false',off), 'togglecache', 'includehomelander', icon=ICONMAINT, themeit=THEME3)   
        if xbmc.getCondVisibility('System.HasAddon(plugin.video.moria)'): addFile('--- Include MORIA: %s' % includemoria.replace('true',on).replace('false',off), 'togglecache', 'includemoria', icon=ICONMAINT, themeit=THEME3)
...
def net_tools(view=None):
    addFile ('Speed Tester' ,'speedtestM', icon=ICONAPK, themeit=THEME1)
    if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
...
def speedTest():
    if not xbmc.getCondVisibility('System.HasAddon(script.speedtester)'):
        xbmc.executebuiltin('ActivateWindow(10001,"plugin://script.speedtester/")')
    elif not xbmc.getCondVisibility('System.AddonIsEnabled(script.speedtester)'):
        xbmc.executebuiltin('EnableAddon(script.speedtester)')
    else:
...
def runSpeedTest():
    try:
        ...
        urllib2.urlretrieve(found[0], dest)
...
def getIP():
    ...
    page  = wiz.openURL(site).decode('utf-8').replace('\n','').replace('\r','')
...
def fixUpdate():
    if KODIV < 17:
        ...
        xbmc.sleep(500)
        xbmc.executeJSONRPC('{"jsonrpc":"2.0","method":"Settings.ResetSettingValue", "params":{"setting":"locale.language"},"id":1}')
        freememory = xbmc.getInfoLabel('System.FreeMemory').replace('MB', '')
        xbmc.sleep(500)
        if int(freememory) <= 1000:
            delay = 10000
        else:
            delay = 5000
        DIALOG.notification("Fix Addons Not Updating", "Please Wait", xbmcgui.NOTIFICATION_INFO, delay, False)
        xbmc.sleep(delay)
        ...
        try:
            ...
            wiz.log("Unable to remove %s, Purging DB" % dbfile)
            wiz.purgeDb(dbfile)
        DIALOG.ok(ADDONTITLE, '[COLOR %s]Please click OK to close kodi and apply the changes[/COLOR]' % COLOR2)
        wiz.killxbmc(True)
...
def buildWizard(name, type, theme=None, over=False):
    ...
    if type == 'gui':
        if name == BUILDNAME:
            if over == True: yes = 1
            else: yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to apply the guifix for:[CR][COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR2, COLOR1, name), nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Apply Fix[/COLOR][/B]')
        else:
        ...
        if yes:
            ...
            if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
            DP.create(ADDONTITLE,'[COLOR %s][B]Downloading GuiFix:[/B][/COLOR] [COLOR %s]%s[/COLOR]Please Wait' % (COLOR2, COLOR1, name))
            lib=os.path.join(PACKAGES, '%s_guisettings.zip' % zipname)
            try: xbmcvfs.delete(lib)
            except: pass
            downloader.download(buildzip, lib, DP, name)
            xbmc.sleep(500)
            title = '[COLOR %s][B]Installing:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name)
            DP.update(0, '%sPlease Wait' % title)
        ...
        if warning == True:
            yes_pressed = DIALOG.yesno("%s - [COLOR red]WARNING!![/COLOR]" % ADDONTITLE, '[COLOR %s]There is a chance that the skin will not appear correctly When installing a %s build on a Kodi %s install [/COLOR][COLOR %s]%s v%s ?[/COLOR]' % (COLOR2, wiz.checkBuild(name, 'kodi'), KODIV, COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR springgreen]No, Cancel[/COLOR][/B]',yeslabel='[B][COLOR red]Yes, Install[/COLOR][/B]')
        else:
            if not over == False: yes_pressed = 1
            else: yes_pressed = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to Download and Install:[COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]No, Cancel[/COLOR][/B]',yeslabel='[B][COLOR springgreen]Yes, Install[/COLOR][/B]')
        if yes_pressed:
            wiz.clearS('build')
            ...
            if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
            bname = '[B][COLOR %s]%s[/COLOR] v%s[/B]' % (wiz.checkBuild(name,'color'), name, wiz.checkBuild(name,'version'))
            DP.create(ADDONTITLE,'[COLOR %s][B]Downloading: %s[/B][/COLOR] [COLOR %s] v%s[/COLOR]Please Wait' % (COLOR2, wiz.checkBuild(name,'color'), name, wiz.checkBuild(name,'version')))
            lib=os.path.join(PACKAGES, '%s.zip' % zipname)
            try: xbmcvfs.delete(lib)
            except: pass
            downloader.download(buildzip, lib, DP, bname)
            xbmc.sleep(500)
            title = '[COLOR %s][B]Installing:[/B][/COLOR] [COLOR %s]%s v%s[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild(name,'version'))
            DP.update(0, '%s[CR][CR] Please Wait' % title)
            percent, errors, error = extract.all(lib,HOME,DP, title=title)
            if int(float(percent)) > 0:
                ...
                if int(float(errors)) > 0:
                    yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s v%s[/COLOR][CR]Done: [COLOR %s]%s%s[/COLOR] [Errors:[COLOR %s]%s[/COLOR]][CR]Would you like to view the errors?[/COLOR]' % (COLOR2, COLOR1, name, wiz.checkBuild(name,'version'), COLOR1, percent, '%', COLOR1, errors), nolabel='[B][COLOR white]No Thanks[/COLOR][/B]', yeslabel='[B][COLOR white]View Errors[/COLOR][/B]')
                    if yes:
        ...
        if installtheme:
            ...
            if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
            DP.create(ADDONTITLE,'[COLOR %s][B]Downloading:[/B][/COLOR] [COLOR %s]%s[/COLOR]Please Wait' % (COLOR2, COLOR1, theme))
            lib=os.path.join(PACKAGES, '%s.zip' % zipname)
            try: xbmcvfs.delete(lib)
            except: pass
            downloader.download(themezip, lib, DP, theme)
            xbmc.sleep(500)
            ...
            if url not in ["fresh", "normal"]:
                ...
                if test2 == True:
                    ...
                    switch = wiz.swapSkins(wiz.getS('defaultskin'), 'Theme Install')
...
def thirdPartyInstall(name, url):
    ...
    type = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to preform a [COLOR %s]Fresh Install[/COLOR] or [COLOR %s]Normal Install[/COLOR] for:[/COLOR][CR][COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, COLOR1, COLOR1, name), yeslabel="[B][COLOR springgreen]Fresh Install[/COLOR][/B]", nolabel="[B][COLOR red]Normal Install[/COLOR][/B]")
    if type == 1:
        freshStart('third', True)
    wiz.clearS('build')
    zipname = name.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', '')
    if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
    DP.create(ADDONTITLE,'[COLOR %s][B]Downloading:[/B][/COLOR] [COLOR %s]%s[/COLOR][CR][CR]Please Wait' % (COLOR2, COLOR1, name))
    lib=os.path.join(PACKAGES, '%s.zip' % zipname)
    try: xbmcvfs.delete(lib)
    except: pass
    downloader.download(url, lib, DP, name)
...
def apkInstaller(apk, url):
    ...
    if wiz.platform() == 'android':
        ...
        if KODIV >= 18:
            wiz.ebi('StartAndroidActivity(,android.intent.action.VIEW,,%s)' % (url))
        else:
            DP.create(ADDONTITLE,'[COLOR %s][B]Downloading:[/B][/COLOR] [COLOR %s]%s[/COLOR][CR][CR]Please Wait!' % (COLOR2, COLOR1, display))
            lib=os.path.join(PACKAGES, "%s.apk" % apk.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', ''))
            try: xbmcvfs.delete(lib)
            except: pass
            downloader.download(url, lib, DP, display)
            xbmc.sleep(100)
            DP.close()
...
def toggleCache(state):
    cachelist = ['includevideo', 'includeall', 'includegaia', 'includethecrew', 'includeseren']
    titlelist = ['Include Video Addons', 'Include All Addons', 'Include Gaia', 'Include The Crew', 'Include Homelander', 'Include Moria', 'Include Seren']
