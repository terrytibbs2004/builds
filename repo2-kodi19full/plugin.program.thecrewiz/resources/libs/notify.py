...
def firstRun():
    class MyWindow(xbmcgui.WindowXMLDialog):
        ...
        def doBuildMenu(self):
            wiz.log("[Check Updates] [User Selected: Open Build Menu] [Next Check: %s]" % str(NEXTCHECK), LOGNOTICE)
        ...
        def doIgnore(self):
            self.close()
            wiz.log("[First Run] [User Selected: Ignore Build Menu] [Next Check: %s]" % str(NEXTCHECK), LOGNOTICE)
            wiz.setS('lastbuildcheck', str(NEXTCHECK))
...
def notification(msg='', test=False):
    class MyWindow(xbmcgui.WindowXMLDialog):
        ...
        def doRemindMeLater(self):
            ...
            wiz.log("[Notification] NotifyID %s Remind Me Later" % wiz.getS('noteid'), LOGNOTICE)
            self.close()

        def doDismiss(self):
            if not test == True:
                wiz.setS("notedismiss","true")
            wiz.log("[Notification] NotifyID %s Dismissed" % wiz.getS('noteid'), LOGNOTICE)
            self.close()
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
    class MyWindow(xbmcgui.WindowXMLDialog):
        ...
        def doFreshInstall(self):
            wiz.log("[Check Updates] [Installed Version: %s] [Current Version: %s] [User Selected: Fresh Install build]" % (BUILDVERSION, LATESTVERSION), LOGNOTICE)
            wiz.log("[Check Updates] [Next Check: %s]" % str(NEXTCHECK), LOGNOTICE)
            wiz.setS('lastbuildcheck', str(NEXTCHECK))
            self.close()
        ...
        def doNormalInstall(self):
            wiz.log("[Check Updates] [Installed Version: %s] [Current Version: %s] [User Selected: Normal Install build]" % (BUILDVERSION, LATESTVERSION), LOGNOTICE)
            wiz.log("[Check Updates] [Next Check: %s]" % str(NEXTCHECK), LOGNOTICE)
            wiz.setS('lastbuildcheck', str(NEXTCHECK))
            self.close()
        ...
        def doIgnore(self):
            wiz.log("[Check Updates] [Installed Version: %s] [Current Version: %s] [User Selected: Ignore 3 Days]" % (BUILDVERSION, LATESTVERSION), LOGNOTICE)
            wiz.log("[Check Updates] [Next Check: %s]" % str(THREEDAYS), LOGNOTICE)
