...
class indexer:
    ...
    def rootextinfo(self):
            regex.clear()
            url = 'https://magnetic.website/nightwing/2021%201-click%20movies.xml'
            self.list = self.construct_mag(url)
            for i in self.list: i.update({'content': 'movies'})
            self.addDirectory(self.list)
            return self.list
    
    def rootlists1(self):
            regex.clear()
            url = 'https://magnetic.website/Mad%20Titan/1-Click%20Tv%20Shows.xml'
            self.list = self.construct_mag(url)
            for i in self.list: i.update({'content': 'movies'})
            self.addDirectory(self.list)
            return self.list
    ...
    def rootlists2(self):
            regex.clear()
            url = 'https://magnetic.website/nightwing/4kmovies.xml'
            self.list = self.construct_mag(url)
            for i in self.list: i.update({'content': 'movies'})
            self.addDirectory(self.list)
            return self.list
    ...
    def rootlists3(self):
            regex.clear()
            url = 'https://magnetic.website/nightwing/kids%201%20click%20movies.xml'
            self.list = self.construct_mag(url)
            for i in self.list: i.update({'content': 'movies'})
            self.addDirectory(self.list)
            return self.list
    ...
    def rootlists4(self):
            regex.clear()
            url = 'https://magnetic.website/nightwing/horror%201%20click.xml'
            self.list = self.construct_mag(url)
            for i in self.list: i.update({'content': 'movies'})
            self.addDirectory(self.list)
            return self.list
    ...
    def rootlists5(self):
            regex.clear()
            url = 'http://magnetic.website/Mad%20Titan/Boxsets.xml'
            self.list = self.construct_mag(url)
            for i in self.list: i.update({'content': 'movies'})
            self.addDirectory(self.list)
            return self.list
    ...
    def rootlists6(self):
            regex.clear()
            url = 'https://magnetic.website/nightwing/ProtoHobb%20Fitness.xml'
            self.list = self.construct_mag(url)
            for i in self.list: i.update({'content': 'movies'})
            self.addDirectory(self.list)
            return self.list  
    ...
    def getx(self, url, worker=False):
            ...
            url = regex.resolve(r)
            self.list = self.construct_mag('', result=url)
            self.addDirectory(self.list)
            return self.list
    ...
    def developer(self):
            ...
            result = f.read()
            f.close()
            self.list = self.construct_mag('', result=result)
            for i in self.list:
    ...
    def construct_mag(self, url, result=None):
            ...
            for item in items:          
                ...
                try: summary = re.findall('<summary>(.+?)</summary>', item)[0]
                except: summary = '0'
                ...
                self.list.append({'name': name, 'vip': vip, 'url': url, 'action': action, 'folder': folder, 'poster': image2, 'banner': '0', 'fanart': fanart2, 'content': content, 'imdb': imdb, 'tvdb': tvdb, 'tmdb': '0', 'title': title, 'originaltitle': title, 'tvshowtitle': tvshowtitle, 'year': year, 'premiered': premiered, 'season': season, 'episode': episode, 'summary': summary})
    ...
    def addDirectory(self, items, queue=False):
        ...
        for i in items:
                ...
                summary = i['summary'] if 'summary' in i else 'No Summary Details'
                
                meta.update({'plot': summary})
                ...
                if queue == False:
                    ...
                    item.setInfo(type='video', infoLabels={'mediatype': 'movie', 'title': name, 'plot': summary})
