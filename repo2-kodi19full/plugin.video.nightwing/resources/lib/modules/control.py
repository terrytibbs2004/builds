...
def artPath():
    theme = appearance()
    return os.path.join(xbmcaddon.Addon('plugin.video.nightwing').getAddonInfo('path'), 'resources', 'media', theme)
...
def clean_settings():#Nightwing code
    ...
    def _make_content(dict_object):
        ...
        return content
    try:
        for addon_id in ('plugin.video.nightwing', 'script.module.nightwingscrapers'):
            removed_settings = []
            active_settings = []
