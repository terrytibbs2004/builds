...
class jetextractors(Plugin):
    ...
    def routes(self, plugin):
        ...
        def get_games(site):
            for e in extractor.get_extractors():
                if site == e.name:
                    ...
                    for game in games:
                        jen_data = {
                            "title": "[COLORdodgerblue]%s[COLORwhite] |[B][I] %s[/B][/I]  [COLORred]%s[/COLOR]" % (game.league.replace("'", "") if game.league is not None else "", game.title, format_time(game.starttime)),
                            "thumbnail": game.icon,
                            "fanart": game.icon,
                            "summary": game.title,
        ...
        def search_games(query):
            ...
            for game in games:
                jen_data = {
                    "title": "[COLORdodgerblue]%s[COLORwhite] |[B][I] %s[/B][/I]  [COLORred]%s | %s[/COLOR]" % (game.league.replace("'", "") if game.league is not None else "", game.title, game.extractor, format_time(game.starttime)),
                    "thumbnail": game.icon,
                    "fanart": game.icon,
                    "summary": game.title,
                    "sportjetextractors": [link.to_dict() for link in game.links],
        ...
        def iptv_setup():
            ...
            try:
                ...
                addon.setSetting(id="m3uUrl", value="https://magnetic1.ratpack.appboxes.co/jet/playlist.m3u8")
                addon.setSetting(id="epgUrl", value="https://magnetic1.ratpack.appboxes.co/jet/epg.xml")
