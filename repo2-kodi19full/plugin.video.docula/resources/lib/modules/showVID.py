...
message = "[B][COLORred]WOOOPS!! Link Is No Longer Available[/COLOR][/B]"
msg2 = "[B][COLORlime]The Bell Has Rung, Getting Ready To Rumble[/COLOR][/B]"
...
#======================================================================================================
    ...
    @staticmethod
    # Get and Play stream
    def getSTREAM(theSTREAM):
 		
        #errorMsg="%s" % (theSTREAM)
        #xbmcgui.Dialog().ok("theSTREAM", errorMsg)
 		
        if theSTREAM is not None:		
            Common.play(theSTREAM)
        else:
            dialog = xbmcgui.Dialog()
            dialog.notification("[B][COLORred]Resolver Error[/COLOR][/B]", message, xbmcgui.NOTIFICATION_INFO, duration)
...
#======================================================================================================
    ...
    def getVID(mode):
        ...
        if stream is not None:		
            dialog = xbmcgui.Dialog()
            dialog.notification("[B][COLORred]Resolver Initiated[/COLOR][/B]", msg2, xbmcgui.NOTIFICATION_INFO, duration)
            Common.play(stream)
        else:
            dialog = xbmcgui.Dialog()
            dialog.notification("[B][COLORred]Resolver Error[/COLOR][/B]", message, xbmcgui.NOTIFICATION_INFO, duration)
