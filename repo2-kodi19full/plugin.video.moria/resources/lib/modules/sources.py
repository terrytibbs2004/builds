...
class sources:
    ...
    def getConstants(self):
        self.itemProperty = 'plugin.video.moria.container.items'

        self.metaProperty = 'plugin.video.moria.container.meta'
        ...
        self.module_name = 'Moria:'

        if externalEnabled == 'true':
            try:
                import moriascrapers
                self.sourceDict += moriascrapers.sources()
                self.module_name = 'MoriaScrapers (' + str(control.addon('script.module.moriascrapers').getSetting('package.folder')) + ' set):' \
                                   if control.addon('script.module.moriascrapers').getSetting('package.folder') != 'Moriascrapers' else '[COLORff58c1e0]MORIA[/COLOR][COLOR ffafafaf] will find your links[/COLOR]'
            except:
                pass
