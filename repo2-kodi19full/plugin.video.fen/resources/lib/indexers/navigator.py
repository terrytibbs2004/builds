...
class Navigator:
	...
	def folder_navigator(self):
		...
		handle = int(sys.argv[1])
		folder_path, folder_id = self.params_get('folder_path'), self.params_get('folder_id')
		assigned_content = gather_assigned_content("'FEN_FOLDERS_%s" % folder_id + "_%'")
		...
		def _builder():
			for item in folder_info:
				for media_type in ('movie', 'tvshow'):
					...
					if folder_path:
						cm = [(brow_str, container_update % build_url({'mode': 'navigator.folder_navigator', 'folder_path': folder_path, 'folder_id': setting_id}))]
						listitem.addContextMenuItems(cm)
					listitem.setLabel(display)
