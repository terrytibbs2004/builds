...
def appearance():
    appearance = setting('theme.1').lower() if condVisibility('System.HasAddon(script.scrubsv2.artwork)') else setting('theme.alt').lower()
    return appearance
...
def moderator():
    netloc = [urllib_parse.urlparse(sys.argv[0]).netloc, '', 'plugin.video.metalliq', 'script.extendedinfo', 'plugin.program.super.favourites', 'plugin.video.themoviedb.helper']
