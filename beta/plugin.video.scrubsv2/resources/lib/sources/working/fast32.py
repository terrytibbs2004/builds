...
class source:
    ...
    def sources(self, url, hostDict):
        try:
            ...
            results = [(client.parseDOM(i, 'a', ret='href'), re.findall('(\d{4})', i), client.parseDOM(i, 'a')) for i in results]
            results = [(i[0][0], i[1][0], i[2][0]) for i in results if len(i[0]) > 0 and len(i[1]) > 0 and len(i[2]) > 0]
            if 'tvshowtitle' in data:
                r = [(i[0], i[1], re.findall('(.+?)(?:\(\d+\)|)(?:\:|) Season (\d+)$', client.replaceHTMLCodes(i[2]))) for i in results]
                ...
                r = [(i[0], i[1], re.findall('(.+?)(?:\(\d+\)|)$', client.replaceHTMLCodes(i[2]))) for i in results]
                r = [(i[0], i[1], i[2][0]) for i in r if len(i[2]) > 0]
